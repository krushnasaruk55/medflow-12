...
app.post('/api/patient-app/register', async (req, res) => {
  ...
  try {
    ...
    if (existing) {
      ...
      existing.password = auth.hashPassword(password); // Re-using auth helper if available or simple
...
app.post('/api/patient-app/login', async (req, res) => {
  ...
  try {
    ...
    for (const p of patients) {
      if (p.password && auth.comparePassword(password, p.password)) {
        validPatient = p;
        break;
      }
    }
...
app.post('/api/ai/generate', async (req, res) => {
  // const hospitalId = req.session.hospitalId; // Optional: Enforce auth
  ...
  if (!apiKey) {
    // Mock Responses so UI works
    let mockResponse = "AI Integration Pending (Add API Keys)";
    if (type === 'prescription') {
      mockResponse = "Tab. Amoxicillin 500mg - 1-0-1 (5 days)\nTab. Paracetamol 650mg - 1-0-1 (3 days)\nSyr. Cough Syrup - 2tsp TDS";
    } else if (type === 'summary') {
      mockResponse = "Patient has a history of hypertension (2020) and mild asthma. Recent visits indicate recurring headaches.";
    } else {
      mockResponse = "I'm your AI assistant. I can help with diagnoses and summaries once connected to an LLM provider.";
    }

    // Return success with mock data but warn in console
    // console.log('⚠️ AI Request received but no keys. Returning mock data.');
    return res.json({ result: mockResponse, isMock: true });
  }

  try {
    // TODO: Implement actual OpenAI/Gemini call here
    // For now, even with key, we return a success message logic
    // const response = await openai.chat.completions.create({...})

    res.json({ result: "Real AI response would go here (Logic to be implemented once keys active)." });
  } catch (error) {
